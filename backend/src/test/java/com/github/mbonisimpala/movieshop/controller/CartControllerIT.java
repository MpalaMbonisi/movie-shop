...
public class CartControllerIT {
    ...
    @Test
    public void shouldReturnCartList_whenGettingExistingAccountCartList() throws Exception{
        // Given an existing account with cart list items
        cartService.saveCartItem(testAccount.getId(), movie01.getId());
        cartService.saveCartItem(testAccount.getId(), movie02.getId());
        cartService.saveCartItem(testAccount.getId(), movie03.getId());

        // When
        MockHttpServletRequestBuilder request = MockMvcRequestBuilders.get("/cart/account/" + testAccount.getId())
    ...
    @Test
    public void shouldReturnEmptyList_whenGettingExistingAccountEmptyCartList() throws Exception{
        // Given an existing account with no movies on cart list
        // When
        MockHttpServletRequestBuilder request = MockMvcRequestBuilders.get("/cart/account/" + testAccount.getId())
                .header("Authorization", "Bearer " + token);
    ...
    @Test
    public void shouldReturnCartItem_whenSavingExistingMovieIdAndAccountId() throws Exception{
        // Given existing Account ID and movie ID
        long accountId = testAccount.getId();
        long movieId = movie01.getId();

        // When
        String url = "/cart/account/" + accountId + "/movie/" + movieId;
    ...
    @Test
    public void shouldReturnNoContent_whenDeletingExistingMovieIdAndAccountId() throws Exception{
        // Given existing Account ID and movie ID
        long accountId = testAccount.getId();
        long movieId = movie01.getId();
    ...
    @Test
    public void shouldReturnNoContent_whenDeletingAllExistingAccountCartItems() throws Exception{
        // Given an existing Account ID
        long accountId = testAccount.getId();

        // When
        String url = "/cart/account/" + accountId + "/clear";
        MockHttpServletRequestBuilder request = MockMvcRequestBuilders.delete(url)
                .header("Authorization", "Bearer " + token);
