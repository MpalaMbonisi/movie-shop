...
public class MyListControllerIT {

    @Autowired
    private MockMvc mockMvc;
    @Autowired
    private OrderedMovieRepository orderedMovieRepository;
    @Autowired
    private OrderedMovieService orderedMovieService;
    @Autowired
    ...
    void cleanup(){
        ...
        orderedMovieRepository.deleteAll();
    ...
    @Test
    public void shouldReturnOrderedMovieList_whenGettingExistingAccountOrderedMovies() throws Exception{
        // Given an existing account with ordered movies
        orderedMovieService.saveOrderedMovie(testAccount.getId(), movie01.getId());
        orderedMovieService.saveOrderedMovie(testAccount.getId(), movie02.getId());
        orderedMovieService.saveOrderedMovie(testAccount.getId(), movie03.getId());

        // When
        MockHttpServletRequestBuilder request = MockMvcRequestBuilders.get("/mylist/account/" + testAccount.getId() +"/all")
    ...
    @Test
    public void shouldReturnEmptyList_whenGettingExistingAccountNoMoviesOrdered() throws Exception{
        // Given an existing account with no ordered movies
        // When
        MockHttpServletRequestBuilder request = MockMvcRequestBuilders.get("/mylist/account/" + testAccount.getId() +"/all")
                .header("Authorization", "Bearer " + token);
    ...
    @Test
    public void shouldReturnOrderedMovie_whenSavingExistingMovieIdAndAccountId() throws Exception{
        // Given existing Account ID and movie ID
        long accountId = testAccount.getId();
        long movieId = movie01.getId();

        // When
        String url = "/mylist/account/" + accountId + "/movie/" + movieId;
